...
```
Alternately a copy of the resulting bgzipped file is provided in Metadata/hs38_chroms.bed.gz (still needs to be tabix-indexed).


## CONFIGURATION:
*****************
Before using the pipeline you must customize (a copy of) the grexomeTIMCprim_config.pm file, which defines every install-specific variable (eg path+name of the reference human genome fasta file, possibly produced following the instructions in "REQUIRED DATA" above). Every subroutine in grexomeTIMCprim_config.pm is self-documented and will need to be customized. To do this you should copy the file somewhere and edit the copy, then use --config. Otherwise you could just edit the distributed copy in-place, although this not as flexible and your customizations will get squashed when you git pull.
